class Interface {
    ...
    listen(input) {
        ...
        if (input.startsWith("add")) {
            ...
            this.addTask(data[0].trim(), data[1].trim(), Number(data[2].trim()), Number(data[3].trim()));
        }
    ...
    addTask(taskname, sector, begin, end) {
        this.update({
            id: randID(5),
            taskname: taskname,
            sector: sector,
...
class Display {
    constructor(inter) {
        ...
        this.command.addEventListener("keydown", ({ key }) => {
            if (key === "Enter") {
                this.interface.listen(this.command.value);
                this.command.value = "";
            }
        });
        this.updateCircle();
